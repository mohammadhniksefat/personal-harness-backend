...
def safe_path(p):
    root=Path(settings.harness_workspace).resolve(); target=(root/p).resolve() if not Path(p).is_absolute() else Path(p).resolve()
    if root != target and root not in target.parents: raise ToolError('Path is outside the configured harness workspace')
    return target
...
def terminal(args, cancel_event):
    ...
    if first not in SAFE_CMDS: raise ToolError(f'Command not allowed by terminal policy: {first}')
    sensitive=any(command.lower().startswith(x) for x in SENSITIVE_PREFIXES)
    if sensitive: raise ToolError('APPROVAL_REQUIRED: This terminal command is classified as sensitive.')
    proc=subprocess.Popen(['powershell.exe','-NoProfile','-NonInteractive','-Command',command],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,cwd=settings.harness_workspace)
    while True:
        if cancel_event.is_set():
            proc.kill(); raise ToolError('CANCELLED_BY_USER')
        line=proc.stdout.readline()
        if line: yield line
        elif proc.poll() is not None: break
    if proc.returncode: raise ToolError(f'Command exited with code {proc.returncode}')
...
def datetime_tool(args, cancel_event): return datetime.now(ZoneInfo(args.get('timezone','UTC'))).isoformat()
def system_info(args,cancel_event): return {'platform':platform.platform(),'python':platform.python_version(),'machine':platform.machine(),'processor':platform.processor(),'cwd':os.getcwd()}

def clipboard(args,cancel_event):
    import tkinter as tk
    root=tk.Tk(); root.withdraw()
    try:
        if args['operation']=='write': root.clipboard_clear(); root.clipboard_append(args['text']); root.update(); return 'Clipboard updated.'
        root.update(); return root.clipboard_get()
...
def database_query(args,cancel_event):
    ...
    raise ToolError('Database query tool is intentionally disabled until an explicit safe-query adapter is configured.')

TOOLS={
 'terminal': {'description':'Run an approved, policy-constrained PowerShell command in the harness workspace.','schema':{'type':'object','properties':{'command':{'type':'string'}},'required':['command']},'risk':'high','fn':terminal},
 'file_read': {'description':'Read a UTF-8 text file inside the harness workspace.','schema':{'type':'object','properties':{'path':{'type':'string'}},'required':['path']},'risk':'medium','fn':file_read},
 'file_write': {'description':'Write UTF-8 text to a file inside the harness workspace.','schema':{'type':'object','properties':{'path':{'type':'string'},'content':{'type':'string'}},'required':['path','content']},'risk':'high','fn':file_write},
 'calculator': {'description':'Calculate a basic arithmetic expression.','schema':{'type':'object','properties':{'expression':{'type':'string'}},'required':['expression']},'risk':'low','fn':calculator},
