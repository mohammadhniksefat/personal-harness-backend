...
def _needs_approval(name, args):
    ...
    sensitive_prefixes = (
        ...
        "git clean", "git reset", "shutdown", "taskkill", "format",
        "del ", "remove-item", "set-itemproperty",
    )
...
async def execute(db, sid, user_text, emit):
    ...
    try:
        for step in range(settings.max_tool_executions + 1):
            ...
            spec = TOOLS[name]
            te = ToolExecution(session_id=sid, run_id=run.id, tool_name=name, arguments=args, status="pending")
            db.add(te)
            db.commit()
            await emit({"type": "tool_requested", "tool": name, "arguments": args, "execution_id": te.id, "risk": spec["risk"]})

            if _needs_approval(name, args):
                approval = Approval(run_id=run.id, tool_execution_id=te.id, reason="Sensitive terminal command requires explicit approval.")
                db.add(approval)
                te.status = "waiting_for_user"
                session.status = "waiting_for_user"
                db.commit()
                await emit({"type": "approval_required", "approval_id": approval.id, "tool": name, "arguments": args, "reason": approval.reason})
                ...
                if approval.status != "approved":
                    ...
                    db.add(Message(session_id=sid, role="tool", content="USER_REJECTED_OPERATION", tool_call_id=call["id"]))
                    session.status = "running"
                    db.commit()
                    await emit({"type": "tool_finished", "execution_id": te.id, "status": "rejected"})
                    continue
                te.approved = True
                session.status = "running"
                db.commit()
            ...
            try:
                ...
                if hasattr(result, "__iter__") and not isinstance(result, (str, dict, list)):
                    ...
                    for x in result:
                        chunks.append(x)
                        await emit({"type": "tool_output", "execution_id": te.id, "data": x})
                    result = "".join(chunks)
                te.result = json.dumps(result) if isinstance(result, (dict, list)) else str(result)
                te.status = "succeeded"
            except Exception as exc:
                te.status = "cancelled" if "CANCELLED_BY_USER" in str(exc) else "failed"
                te.error = str(exc)
                result = {"status": te.status, "error": str(exc)}
                await emit({"type": "tool_failed", "execution_id": te.id, "error": str(exc)})
            db.commit()

            db.add(Message(session_id=sid, role="tool", content=json.dumps(result), tool_call_id=call["id"]))
            db.commit()
            await emit({"type": "tool_finished", "execution_id": te.id, "status": te.status})

        raise ToolError("Execution limit reached")
    except Exception as exc:
        session.status = "cancelled" if "CANCELLED_BY_USER" in str(exc) else "failed"
        db.commit()
        await emit({"type": "run_cancelled" if session.status == "cancelled" else "run_failed", "run_id": run.id, "error": str(exc)})
    finally:
