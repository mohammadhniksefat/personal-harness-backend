...
class Message(Base):
    __tablename__='messages'; id: Mapped[int]=mapped_column(primary_key=True); session_id: Mapped[int]=mapped_column(ForeignKey('sessions.id')); role: Mapped[str]=mapped_column(String(30)); content: Mapped[str]=mapped_column(Text); tool_call_id: Mapped[str|None]=mapped_column(String(100),nullable=True); created_at: Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now)
